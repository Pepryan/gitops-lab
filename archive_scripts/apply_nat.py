...
def delete_prerouting():
    ...
    for i in output.splitlines():
        # split, then get second value at dpt:<port>
        lab7_forward_ports.append(int(i.split(':', 1)[1]))
    for port in lab7_forward_ports:
        str_port = str(port)
        id_ports = subprocess.getoutput('sudo iptables -t nat -v -L PREROUTING -n --line-number | grep ' + str_port + ' | awk "{print $1}"')
...
def apply_changes(file, file_cfg):
    for port in file:
...
apply_changes(file_forward_ports, file_cfg)
